...
(function () {
  ...
  // UBAH RAW NI IKUT FILE ORIGINAL KAU
  ...
  // LAYER 1: CHECK INJECTION
  function checkInjection() {
    const nodeOptions = process.env.NODE_OPTIONS || "";
    const execArgv = process.execArgv.join(" ");

    const bad = ["--require", "--loader", "--inspect", "--inspect-brk", "--import"];

    for (const flag of bad) {
      if (nodeOptions.includes(flag)) stop();
      if (execArgv.includes(flag)) stop();
    }
  }

  // LAYER 2: CHECK PATCH RUNTIME
  function checkRuntimePatch() {
    if (process.exit !== SAFE.exit) stop();
    if (process.kill !== SAFE.kill) stop();
    if (process.abort !== SAFE.abort) stop();
    if (fs.readFileSync !== SAFE.readFileSync) stop();
    if (fs.existsSync !== SAFE.existsSync) stop();
    if (crypto.createHash !== SAFE.createHash) stop();
    if (https.get !== SAFE.httpsGet) stop();
    if (global.setInterval !== SAFE.setInterval) stop();
    if (global.setTimeout !== SAFE.setTimeout) stop();
  }

  // LAYER 3: CHECK LOCAL FILE
  function checkLocalFile() {
    if (!SAFE.existsSync.call(fs, __filename)) stop();
    return SAFE.readFileSync.call(fs, __filename);
  }

  // LAYER 4: CHECK LOCAL VS GITHUB RAW
  async function checkGithubSHA() {
    ...
    if (localSHA !== remoteSHA) stop();
  }

  checkGithubSHA().catch(() => stop());
