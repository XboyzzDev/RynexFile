...
(function () {
  ...
  function stop(reason = "UNKNOWN") {
    if (DEBUG_SECURITY) console.log("RYNEX SECURITY STOPPED:", reason);
  ...
  async function checkGithubSHA() {
    if (!SAFE.existsSync.call(fs, __filename)) {
      return stop("LOCAL INDEX.JS MISSING");
    }

    const localFile = SAFE.readFileSync.call(fs, __filename);
    ...
    if (localSHA !== remoteSHA) {
      return stop("LOCAL SHA TAK SAMA DENGAN GITHUB RAW");
    }
  }
